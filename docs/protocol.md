...
with `Microsoft Word Document` further down the file, after every alias and glob line. And
`application/x-ms-dos-executable.xml`, the type behind the `"Data"` fallback above, carries
zero `<comment>` elements of any language at all, which is why `Kinds::comment` returns
`Option<String>` rather than an empty string.
...
`globs2` have no entry, so the execute bits in `p` decide between `application-x-executable`
and `application-x-generic`. Without that rule every one of the 190 drew as a program, which is
